"""Sensor platform for Pollen Levels with language support.""" import logging from datetime import timedelta

import aiohttp from homeassistant.helpers.update_coordinator import DataUpdateCoordinator, UpdateFailed from homeassistant.helpers.entity import Entity from homeassistant.const import ATTR_ATTRIBUTION

from .const import ( CONF_API_KEY, CONF_LATITUDE, CONF_LONGITUDE, CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL, CONF_LANGUAGE_CODE, DOMAIN, )

_LOGGER = logging.getLogger(name)

Icons mapping

TYPE_ICONS = {"GRASS": "mdi:grass", "TREE": "mdi:tree", "WEED": "mdi:flower-tulip"} PLANT_TYPE_ICONS = TYPE_ICONS DEFAULT_ICON = "mdi:flower-pollen"

async def async_setup_entry(hass, entry, async_add_entities): """Set up sensors grouped by type and plant.""" api_key = entry.data[CONF_API_KEY] lat = entry.data[CONF_LATITUDE] lon = entry.data[CONF_LONGITUDE] interval = entry.data.get(CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL) lang = entry.data.get(CONF_LANGUAGE_CODE)
...
class PollenDataUpdateCoordinator(DataUpdateCoordinator): """Coordinator to fetch pollen data with selected language."""
...
class PollenSensor(Entity): """Sensor for an individual pollen code."""

def __init__(self, coordinator: PollenDataUpdateCoordinator, code: str):
    self.coordinator = coordinator
    self.code = code
    # entity_id must be set here before HA assigns
    self.entity_id = f"sensor.{DOMAIN}_{self.code}"
