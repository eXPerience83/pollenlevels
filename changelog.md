...
- **Config step parity**: the initial setup now allows an **empty** `language_code` (same as Options). When empty, we skip validation and **do not send** `languageCode` to the API during the connectivity probe.
